...
class FrozenT5Embedder(AbstractEncoder):
    ...
    def __init__(self, version="google/t5-v1_1-large", device="mps", max_length=77, freeze=True):  # others are google/t5-v1_1-xl and google/t5-v1_1-xxl
        super().__init__()
        self.tokenizer = T5Tokenizer.from_pretrained(version, device=device)
        self.transformer = T5EncoderModel.from_pretrained(version, device=device)
        self.device = device
...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, arch="ViT-H-14", version="laion2b_s32b_b79k", device="mps", max_length=77,
        ...
        super().__init__()
        assert layer in self.LAYERS
        model, _, _ = open_clip.create_model_and_transforms(arch, device=torch.device("mps"), pretrained=version)
        del model.visual
        self.model = model
